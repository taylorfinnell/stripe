...
describe "SubscriptionsApi" do
  ...
  # @option opts [Bool] :invoice_now Will generate a final invoice that invoices for any un-invoiced metered usage and new/pending proration invoice items. Defaults to &#x60;false&#x60;.
  ...
  # @option opts [Int32] :backdate_start_date A past timestamp to backdate the subscription&#39;s start date to. If set, the first invoice will contain line items for the timespan between the start date and the current time. Can be combined with trials and the billing cycle anchor.
  ...
  # @option opts [BillingMode] :billing_mode
