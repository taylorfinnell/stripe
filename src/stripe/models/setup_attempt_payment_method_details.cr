#  Stripe API
#
# The Stripe REST API. Please see https://stripe.com/docs/api for more details.
#
# The version of the OpenAPI document: 2025-06-30.basil
# Contact: dev-platform@stripe.com
# Generated by: https://openapi-generator.tech
#

require "../../core"

require "./setup_attempt_payment_method_details_bancontact" 
require "./setup_attempt_payment_method_details_card" 
require "./setup_attempt_payment_method_details_card_present" 
require "./setup_attempt_payment_method_details_ideal" 
require "./setup_attempt_payment_method_details_naver_pay" 
require "./setup_attempt_payment_method_details_sofort" 

module Stripe
  # 
  class SetupAttemptPaymentMethodDetails
    include JSON::Serializable
    include JSON::Serializable::Unmapped
    include OpenApi::Validatable
    include OpenApi::Json

    # Required Properties

    # The type of the payment method used in the SetupIntent (e.g., `card`). An additional hash is included on `payment_method_details` with a name matching this value. It contains confirmation-specific information for the payment method.
    @[JSON::Field(key: "type", type: String?, default: nil, required: true, nullable: false, emit_null: false)]
    getter _type : String? = nil
    MAX_LENGTH_FOR__TYPE = 5000


    # End of Required Properties

    # Optional Properties

    # 
    @[JSON::Field(key: "acss_debit", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter acss_debit : JSON::Any? = nil



    # 
    @[JSON::Field(key: "amazon_pay", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter amazon_pay : JSON::Any? = nil



    # 
    @[JSON::Field(key: "au_becs_debit", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter au_becs_debit : JSON::Any? = nil



    # 
    @[JSON::Field(key: "bacs_debit", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter bacs_debit : JSON::Any? = nil



    @[JSON::Field(key: "bancontact", type: Stripe::SetupAttemptPaymentMethodDetailsBancontact?, default: nil, required: false, nullable: false, emit_null: false)]
    getter bancontact : Stripe::SetupAttemptPaymentMethodDetailsBancontact? = nil



    # 
    @[JSON::Field(key: "boleto", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter boleto : JSON::Any? = nil



    @[JSON::Field(key: "card", type: Stripe::SetupAttemptPaymentMethodDetailsCard?, default: nil, required: false, nullable: false, emit_null: false)]
    getter card : Stripe::SetupAttemptPaymentMethodDetailsCard? = nil



    @[JSON::Field(key: "card_present", type: Stripe::SetupAttemptPaymentMethodDetailsCardPresent?, default: nil, required: false, nullable: false, emit_null: false)]
    getter card_present : Stripe::SetupAttemptPaymentMethodDetailsCardPresent? = nil



    # 
    @[JSON::Field(key: "cashapp", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter cashapp : JSON::Any? = nil



    @[JSON::Field(key: "ideal", type: Stripe::SetupAttemptPaymentMethodDetailsIdeal?, default: nil, required: false, nullable: false, emit_null: false)]
    getter ideal : Stripe::SetupAttemptPaymentMethodDetailsIdeal? = nil



    # 
    @[JSON::Field(key: "kakao_pay", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter kakao_pay : JSON::Any? = nil



    # 
    @[JSON::Field(key: "klarna", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter klarna : JSON::Any? = nil



    # 
    @[JSON::Field(key: "kr_card", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter kr_card : JSON::Any? = nil



    # 
    @[JSON::Field(key: "link", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter link : JSON::Any? = nil



    @[JSON::Field(key: "naver_pay", type: Stripe::SetupAttemptPaymentMethodDetailsNaverPay?, default: nil, required: false, nullable: false, emit_null: false)]
    getter naver_pay : Stripe::SetupAttemptPaymentMethodDetailsNaverPay? = nil



    # 
    @[JSON::Field(key: "nz_bank_account", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter nz_bank_account : JSON::Any? = nil



    # 
    @[JSON::Field(key: "paypal", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter paypal : JSON::Any? = nil



    # 
    @[JSON::Field(key: "revolut_pay", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter revolut_pay : JSON::Any? = nil



    # 
    @[JSON::Field(key: "sepa_debit", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter sepa_debit : JSON::Any? = nil



    @[JSON::Field(key: "sofort", type: Stripe::SetupAttemptPaymentMethodDetailsSofort?, default: nil, required: false, nullable: false, emit_null: false)]
    getter sofort : Stripe::SetupAttemptPaymentMethodDetailsSofort? = nil



    # 
    @[JSON::Field(key: "us_bank_account", type: JSON::Any?, default: nil, required: false, nullable: false, emit_null: false)]
    getter us_bank_account : JSON::Any? = nil



    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(
      *,
      # Required properties
      @_type : String? = nil,
      # Optional properties
      @acss_debit : JSON::Any? = nil,
      @amazon_pay : JSON::Any? = nil,
      @au_becs_debit : JSON::Any? = nil,
      @bacs_debit : JSON::Any? = nil,
      @bancontact : Stripe::SetupAttemptPaymentMethodDetailsBancontact? = nil,
      @boleto : JSON::Any? = nil,
      @card : Stripe::SetupAttemptPaymentMethodDetailsCard? = nil,
      @card_present : Stripe::SetupAttemptPaymentMethodDetailsCardPresent? = nil,
      @cashapp : JSON::Any? = nil,
      @ideal : Stripe::SetupAttemptPaymentMethodDetailsIdeal? = nil,
      @kakao_pay : JSON::Any? = nil,
      @klarna : JSON::Any? = nil,
      @kr_card : JSON::Any? = nil,
      @link : JSON::Any? = nil,
      @naver_pay : Stripe::SetupAttemptPaymentMethodDetailsNaverPay? = nil,
      @nz_bank_account : JSON::Any? = nil,
      @paypal : JSON::Any? = nil,
      @revolut_pay : JSON::Any? = nil,
      @sepa_debit : JSON::Any? = nil,
      @sofort : Stripe::SetupAttemptPaymentMethodDetailsSofort? = nil,
      @us_bank_account : JSON::Any? = nil
    )
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties : Array(String)
      invalid_properties = Array(String).new

      invalid_properties.push("\"_type\" is required and cannot be null") if @_type.nil?

      unless (__type = @_type).nil?
        if max_length_error = OpenApi::PrimitiveValidator.max_length_error("_type", __type.to_s.size, MAX_LENGTH_FOR__TYPE)
          invalid_properties.push(max_length_error)
        end
      end
      unless (_acss_debit = @acss_debit).nil?
      end
      unless (_amazon_pay = @amazon_pay).nil?
      end
      unless (_au_becs_debit = @au_becs_debit).nil?
      end
      unless (_bacs_debit = @bacs_debit).nil?
      end
      unless (_bancontact = @bancontact).nil?
        invalid_properties.concat(_bancontact.list_invalid_properties_for("bancontact")) if _bancontact.is_a?(OpenApi::Validatable)
      end
      unless (_boleto = @boleto).nil?
      end
      unless (_card = @card).nil?
        invalid_properties.concat(_card.list_invalid_properties_for("card")) if _card.is_a?(OpenApi::Validatable)
      end
      unless (_card_present = @card_present).nil?
        invalid_properties.concat(_card_present.list_invalid_properties_for("card_present")) if _card_present.is_a?(OpenApi::Validatable)
      end
      unless (_cashapp = @cashapp).nil?
      end
      unless (_ideal = @ideal).nil?
        invalid_properties.concat(_ideal.list_invalid_properties_for("ideal")) if _ideal.is_a?(OpenApi::Validatable)
      end
      unless (_kakao_pay = @kakao_pay).nil?
      end
      unless (_klarna = @klarna).nil?
      end
      unless (_kr_card = @kr_card).nil?
      end
      unless (_link = @link).nil?
      end
      unless (_naver_pay = @naver_pay).nil?
        invalid_properties.concat(_naver_pay.list_invalid_properties_for("naver_pay")) if _naver_pay.is_a?(OpenApi::Validatable)
      end
      unless (_nz_bank_account = @nz_bank_account).nil?
      end
      unless (_paypal = @paypal).nil?
      end
      unless (_revolut_pay = @revolut_pay).nil?
      end
      unless (_sepa_debit = @sepa_debit).nil?
      end
      unless (_sofort = @sofort).nil?
        invalid_properties.concat(_sofort.list_invalid_properties_for("sofort")) if _sofort.is_a?(OpenApi::Validatable)
      end
      unless (_us_bank_account = @us_bank_account).nil?
      end
      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid? : Bool

      return false if @_type.nil?
      unless (__type = @_type).nil?
        return false if __type.to_s.size > MAX_LENGTH_FOR__TYPE
      end

      unless (_acss_debit = @acss_debit).nil?
      end

      unless (_amazon_pay = @amazon_pay).nil?
      end

      unless (_au_becs_debit = @au_becs_debit).nil?
      end

      unless (_bacs_debit = @bacs_debit).nil?
      end

      unless (_bancontact = @bancontact).nil?
        return false if _bancontact.is_a?(OpenApi::Validatable) && !_bancontact.valid?
      end

      unless (_boleto = @boleto).nil?
      end

      unless (_card = @card).nil?
        return false if _card.is_a?(OpenApi::Validatable) && !_card.valid?
      end

      unless (_card_present = @card_present).nil?
        return false if _card_present.is_a?(OpenApi::Validatable) && !_card_present.valid?
      end

      unless (_cashapp = @cashapp).nil?
      end

      unless (_ideal = @ideal).nil?
        return false if _ideal.is_a?(OpenApi::Validatable) && !_ideal.valid?
      end

      unless (_kakao_pay = @kakao_pay).nil?
      end

      unless (_klarna = @klarna).nil?
      end

      unless (_kr_card = @kr_card).nil?
      end

      unless (_link = @link).nil?
      end

      unless (_naver_pay = @naver_pay).nil?
        return false if _naver_pay.is_a?(OpenApi::Validatable) && !_naver_pay.valid?
      end

      unless (_nz_bank_account = @nz_bank_account).nil?
      end

      unless (_paypal = @paypal).nil?
      end

      unless (_revolut_pay = @revolut_pay).nil?
      end

      unless (_sepa_debit = @sepa_debit).nil?
      end

      unless (_sofort = @sofort).nil?
        return false if _sofort.is_a?(OpenApi::Validatable) && !_sofort.valid?
      end

      unless (_us_bank_account = @us_bank_account).nil?
      end

      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] _type Object to be assigned
    def _type=(new_value : String?)

      raise ArgumentError.new("\"_type\" is required and cannot be null") if new_value.nil?
      unless new_value.nil?
        OpenApi::PrimitiveValidator.validate_max_length("_type", new_value.to_s.size, MAX_LENGTH_FOR__TYPE)
      end

      @_type = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] acss_debit Object to be assigned
    def acss_debit=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @acss_debit = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] amazon_pay Object to be assigned
    def amazon_pay=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @amazon_pay = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] au_becs_debit Object to be assigned
    def au_becs_debit=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @au_becs_debit = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] bacs_debit Object to be assigned
    def bacs_debit=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @bacs_debit = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] bancontact Object to be assigned
    def bancontact=(new_value : Stripe::SetupAttemptPaymentMethodDetailsBancontact?)

      unless new_value.nil?
        new_value.validate if new_value.is_a?(OpenApi::Validatable)
      end

      @bancontact = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] boleto Object to be assigned
    def boleto=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @boleto = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] card Object to be assigned
    def card=(new_value : Stripe::SetupAttemptPaymentMethodDetailsCard?)

      unless new_value.nil?
        new_value.validate if new_value.is_a?(OpenApi::Validatable)
      end

      @card = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] card_present Object to be assigned
    def card_present=(new_value : Stripe::SetupAttemptPaymentMethodDetailsCardPresent?)

      unless new_value.nil?
        new_value.validate if new_value.is_a?(OpenApi::Validatable)
      end

      @card_present = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] cashapp Object to be assigned
    def cashapp=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @cashapp = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] ideal Object to be assigned
    def ideal=(new_value : Stripe::SetupAttemptPaymentMethodDetailsIdeal?)

      unless new_value.nil?
        new_value.validate if new_value.is_a?(OpenApi::Validatable)
      end

      @ideal = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] kakao_pay Object to be assigned
    def kakao_pay=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @kakao_pay = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] klarna Object to be assigned
    def klarna=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @klarna = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] kr_card Object to be assigned
    def kr_card=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @kr_card = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] link Object to be assigned
    def link=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @link = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] naver_pay Object to be assigned
    def naver_pay=(new_value : Stripe::SetupAttemptPaymentMethodDetailsNaverPay?)

      unless new_value.nil?
        new_value.validate if new_value.is_a?(OpenApi::Validatable)
      end

      @naver_pay = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] nz_bank_account Object to be assigned
    def nz_bank_account=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @nz_bank_account = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] paypal Object to be assigned
    def paypal=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @paypal = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] revolut_pay Object to be assigned
    def revolut_pay=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @revolut_pay = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] sepa_debit Object to be assigned
    def sepa_debit=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @sepa_debit = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] sofort Object to be assigned
    def sofort=(new_value : Stripe::SetupAttemptPaymentMethodDetailsSofort?)

      unless new_value.nil?
        new_value.validate if new_value.is_a?(OpenApi::Validatable)
      end

      @sofort = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] us_bank_account Object to be assigned
    def us_bank_account=(new_value : JSON::Any?)

      unless new_value.nil?
      end

      @us_bank_account = new_value
    end

    # Generates #hash and #== methods from all fields
    # #== @return [Bool]
    # #hash calculates hash code according to all attributes.
    # #hash @return [UInt64] Hash code
    def_equals_and_hash(@_type, @acss_debit, @amazon_pay, @au_becs_debit, @bacs_debit, @bancontact, @boleto, @card, @card_present, @cashapp, @ideal, @kakao_pay, @klarna, @kr_card, @link, @naver_pay, @nz_bank_account, @paypal, @revolut_pay, @sepa_debit, @sofort, @us_bank_account)
  end
end
