#  Stripe API
#
# The Stripe REST API. Please see https://stripe.com/docs/api for more details.
#
# The version of the OpenAPI document: 2025-06-30.basil
# Contact: dev-platform@stripe.com
# Generated by: https://openapi-generator.tech
#

require "../../core"


module Stripe
  # 
  class IssuingPersonalizationDesignPreferences
    include JSON::Serializable
    include JSON::Serializable::Unmapped
    include OpenApi::Validatable
    include OpenApi::Json

    # Required Properties

    # Whether we use this personalization design to create cards when one isn't specified. A connected account uses the Connect platform's default design if no personalization design is set as the default design.
    @[JSON::Field(key: "is_default", type: Bool?, default: nil, required: true, nullable: false, emit_null: false)]
    getter is_default : Bool? = nil


    # End of Required Properties

    # Optional Properties

    # Whether this personalization design is used to create cards when one is not specified and a default for this connected account does not exist.
    @[JSON::Field(key: "is_platform_default", type: Bool?, default: nil, required: false, nullable: true, emit_null: true, presence: true, ignore_serialize: is_platform_default.nil? && !is_platform_default_present?)]
    getter is_platform_default : Bool? = nil

    @[JSON::Field(ignore: true)]
    property? is_platform_default_present : Bool = false


    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(
      *,
      # Required properties
      @is_default : Bool? = nil,
      # Optional properties
      @is_platform_default : Bool? = nil
    )
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties : Array(String)
      invalid_properties = Array(String).new

      invalid_properties.push("\"is_default\" is required and cannot be null") if @is_default.nil?

      unless (_is_default = @is_default).nil?
      end
      unless (_is_platform_default = @is_platform_default).nil?
      end
      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid? : Bool

      return false if @is_default.nil?
      unless (_is_default = @is_default).nil?
      end

      unless (_is_platform_default = @is_platform_default).nil?
      end

      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] is_default Object to be assigned
    def is_default=(new_value : Bool?)

      raise ArgumentError.new("\"is_default\" is required and cannot be null") if new_value.nil?
      unless new_value.nil?
      end

      @is_default = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] is_platform_default Object to be assigned
    def is_platform_default=(new_value : Bool?)

      unless new_value.nil?
      end

      @is_platform_default = new_value
    end

    # Generates #hash and #== methods from all fields
    # #== @return [Bool]
    # #hash calculates hash code according to all attributes.
    # #hash @return [UInt64] Hash code
    def_equals_and_hash(@is_default, @is_platform_default, @is_platform_default_present)
  end
end
