#  Stripe API
#
# The Stripe REST API. Please see https://stripe.com/docs/api for more details.
#
# The version of the OpenAPI document: 2025-06-30.basil
# Contact: dev-platform@stripe.com
# Generated by: https://openapi-generator.tech
#

require "../../core"

require "./credit_note_refund_refund" 

module Stripe
  # 
  class CreditNoteRefund
    include JSON::Serializable
    include JSON::Serializable::Unmapped
    include OpenApi::Validatable
    include OpenApi::Json

    # Required Properties

    # Amount of the refund that applies to this credit note, in cents (or local equivalent).
    @[JSON::Field(key: "amount_refunded", type: Int32?, default: nil, required: true, nullable: false, emit_null: false)]
    getter amount_refunded : Int32? = nil



    @[JSON::Field(key: "refund", type: Stripe::CreditNoteRefundRefund?, default: nil, required: true, nullable: false, emit_null: false)]
    getter refund : Stripe::CreditNoteRefundRefund? = nil


    # End of Required Properties

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(
      *,
      # Required properties
      @amount_refunded : Int32? = nil, 
      @refund : Stripe::CreditNoteRefundRefund? = nil
    )
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties : Array(String)
      invalid_properties = Array(String).new

      invalid_properties.push("\"amount_refunded\" is required and cannot be null") if @amount_refunded.nil?

      unless (_amount_refunded = @amount_refunded).nil?
      end
      invalid_properties.push("\"refund\" is required and cannot be null") if @refund.nil?

      unless (_refund = @refund).nil?
        invalid_properties.concat(_refund.list_invalid_properties_for("refund")) if _refund.is_a?(OpenApi::Validatable)
      end
      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid? : Bool

      return false if @amount_refunded.nil?
      unless (_amount_refunded = @amount_refunded).nil?
      end

      return false if @refund.nil?
      unless (_refund = @refund).nil?
        return false if _refund.is_a?(OpenApi::Validatable) && !_refund.valid?
      end

      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] amount_refunded Object to be assigned
    def amount_refunded=(new_value : Int32?)

      raise ArgumentError.new("\"amount_refunded\" is required and cannot be null") if new_value.nil?
      unless new_value.nil?
      end

      @amount_refunded = new_value
    end
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] refund Object to be assigned
    def refund=(new_value : Stripe::CreditNoteRefundRefund?)

      raise ArgumentError.new("\"refund\" is required and cannot be null") if new_value.nil?
      unless new_value.nil?
        new_value.validate if new_value.is_a?(OpenApi::Validatable)
      end

      @refund = new_value
    end

    # Generates #hash and #== methods from all fields
    # #== @return [Bool]
    # #hash calculates hash code according to all attributes.
    # #hash @return [UInt64] Hash code
    def_equals_and_hash(@amount_refunded, @refund)
  end
end
